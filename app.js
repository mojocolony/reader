...
function repairSettings(){
  ...
  if(!['light','sepia','grey','dark','eink'].includes(settings.theme)){settings.theme='light';changed=true}
  return changed;
}
...
function filteredArticles(){const q=$('#searchInput').value.trim().toLowerCase();let arr=articles.filter(a=>{if(currentFolder)return a.folderId===currentFolder&&!a.archived;if(currentView==='all')return !a.archived;if(currentView==='favorites')return a.favorite&&!a.archived;if(currentView==='archive')return a.archived;return !a.archived&&!a.folderId});if(q)arr=arr.filter(a=>[a.title,a.excerpt,a.siteName,a.byline,a.textContent].some(v=>(v||'').toLowerCase().includes(q)));const sort=$('#sortSelect').value;arr.sort((a,b)=>sort==='oldest'?a.savedAt-b.savedAt:sort==='title'?a.title.localeCompare(b.title):sort==='source'?(a.siteName||'').localeCompare(b.siteName||''):b.savedAt-a.savedAt);return arr}
...
function renderSidebar(){
  const inbox=articles.filter(a=>!a.archived&&!a.folderId).length,all=articles.filter(a=>!a.archived).length,fav=articles.filter(a=>a.favorite&&!a.archived).length,arc=articles.filter(a=>a.archived).length;
  $('#inboxCount').textContent=inbox||'';$('#allCount').textContent=all||'';$('#favoriteCount').textContent=fav||'';$('#archiveCount').textContent=arc||'';
...
function renderList(){
  const arr=filteredArticles();const title=currentFolder?(folderById(currentFolder)?.name||'Folder'):{inbox:'Inbox',all:'All Articles',favorites:'Favorites',archive:'Archive'}[currentView];
  $('#viewTitle').textContent=title;$('#viewSubtitle').textContent=`${arr.length} article${arr.length===1?'':'s'}`;
  $('#articleList').innerHTML=arr.length?arr.map(a=>{const folderLabel=currentView==='all'&&!currentFolder?esc(a.folderId?(folderById(a.folderId)?.name||'Folder'):'Inbox'):'';return `<div class="article-row ${a.id===currentId?'active':''}" data-id="${a.id}" draggable="true"><div class="article-drag" title="Drag to a folder" aria-hidden="true"><i data-lucide="grip-vertical"></i></div><div class="article-row-body"><div class="article-row-title">${a.favorite?'<span class="fav"><i data-lucide="star"></i></span> ':''}${esc(a.title)}</div><div class="article-row-excerpt">${esc(a.excerpt||'')}</div><div class="article-row-meta">${folderLabel?`<span class="article-folder-label">${folderLabel}</span><span>·</span>`:''}<span>${esc(a.siteName||hostOf(a.url)||'Saved article')}</span><span>·</span><span>${readingMinutes(a)} min</span><span>·</span><span>${formatAge(a.savedAt)}</span></div></div></div>`}).join(''):`<div class="empty-reader" style="height:auto;padding-top:60px"><p>No articles here yet.</p></div>`;
  refreshIcons();
...
async function finishStartupSync(){
  try{
    ...
    // Avoid creating another Welcome article before a connected Dropbox library
    // has had a chance to restore the real library on a new/local-empty device.
    if(!articles.length)await createWelcome();
  }catch(e){
    console.error('Reader background startup sync failed',e);
    if(!articles.length&&!dbx.connected)await createWelcome();
  }
}
async function init(){
  loadLocal();
  ...
  // v1.26.1: render the local library immediately. Dropbox reconciliation runs
  // afterward so a network round-trip can never hold the visible app hostage.
  applySettings();
  renderAll();
  handleCapture();
  wire();
...
function wire(){
  $$('.nav-item').forEach(b=>b.onclick=()=>{currentFolder=null;currentView=b.dataset.view;if(currentView==='all')$('#sortSelect').value='newest';renderAll();if(innerWidth<=900){$('#sidebar').classList.remove('open');$('#backdrop').hidden=true}});
